...
public class ManuscriptServiceImpl implements ManuscriptService {
		...
		try {
			...
			if (StringUtils.isNotEmpty(jsonResultString) && !"UnSuccessful".equals(jsonResultString)) {
				try {
					...
					log.error(
							"Exception caught in getManuscriptDetail() and Convert to Array Liat manuscriptDetailList : "
									+ e.getMessage());
				}
			}
	...
	private String getmanuscriptAPIUrl(HttpGatewayRequestInfo requestInfo, String jCode, int startIndex, int endIndex,
			String status) throws HttpCommunicationException {
		String manuscriptAPIUrl = null;
		int totalCount = 0;
		...
		if (totalCount > 0) {
			...
			if (StringUtils.isNotBlank(status) && status.equals("false")) {
				...
				manuscriptAPIUrl = manauscriptAPIUrl + "onlineFirst/pending/" + jCode + "/" + startIndex + "/"
						+ endIndex;
			}
		}
	...
	private String getmanuscriptCountAPIUrl(HttpGatewayRequestInfo requestInfo, String jCode, String status)
	...
	public ManuscriptDetail getManuscriptTotalCountDetail(String jCode) {
		...
		try {
			// create request
			HttpGatewayRequestInfo requestInfo = new HttpGatewayRequestInfo();
			String manuscriptAPIUrl = manuscriptTotalCountUrl
					+ (StringUtils.isNotEmpty(jCode) ? "journalCode/" + jCode : "manuscript");
			requestInfo.setRequestUri(manuscriptAPIUrl);
			requestInfo.setCharset(Constants.DEFAULT_CHARSET);
			requestInfo.setMethodType(Constants.GET);
			requestInfo.getHeaders().put("MPS-API-TOKEN", manuscriptAPIToken);
	...
	private String getSellerProfileAPIUrl(HttpGatewayRequestInfo requestInfo, String requestUrl, int startIndex,
			int endIndex, String status, SallerProfileDto dto) throws HttpCommunicationException {
		String urlAPI = null;
		int totalCount = 0;
		try {
	...
	public List<CommonResponseItemDto> getStockDetailList(String jCode, int startIndex, int endIndex, String status) {
		...
		try {
			...
			if (StringUtils.isNotEmpty(jsonResultString) && !"UnSuccessful".equals(jsonResultString)) {
				try {
					// Collection<CommonResponsesDto12> readValues = new
					// ObjectMapper().readValue(jsonResultString, new
					// TypeReference<Collection<CommonResponsesDto12>>() { });
					TypeReference<List<CommonResponseDto>> mapType = new TypeReference<List<CommonResponseDto>>() {
					};
					// CommonResponseItemDto commonResponseDto =
					// objectMapper.readValue(jsonResultString.getBytes(),
					// CommonResponseItemDto.class);
